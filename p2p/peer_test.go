...
func testPeer(protos []Protocol) (func(), *conn, *Peer, <-chan error) {
	fd1, fd2 := net.Pipe()
	c1 := &conn{fd: fd1, transport: newTestTransport(&newkey().PublicKey, fd1, &newkey().PublicKey)}
	c2 := &conn{fd: fd2, transport: newTestTransport(&newkey().PublicKey, fd2, &newkey().PublicKey)}
