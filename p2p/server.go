...
func (srv *Server) setupDialScheduler() {
	...
	//节点监控
	var (
		peers = make(map[enode.IDv0]*Peer)
	)
	monitorTaskDoneFurtherFn := func(node *enode.Node) bool {
		srv.log.Trace("disconnect monitor node from ", "node", node)
		srv.dialsched.removeMonitorTask(node)
		if p, ok := peers[node.IDv0()]; ok {
			//关闭仅是monitor的连接
			//MONITOR：记录
			SaveNodePingResult(node.IDv0(), p.RemoteAddr().String(), 1)
			if p.rw.is(monitorConn) && !p.rw.is(dynDialedConn|staticDialedConn|trustedConn|consensusDialedConn|inboundConn) {
				log.Info("disconnect monitor node from, node is only for monitor purpose", "node", node)
				p.Disconnect(DiscRequested)
				return true
			}
		} else {
			SaveNodePingResult(node.IDv0(), "nil", 2)
		}
		return false
	}
	srv.dialsched.initMonitorTaskDoneFurtherFn(monitorTaskDoneFurtherFn)
}
...
func (srv *Server) watching() {
	events := srv.eventMux.Subscribe(cbfttypes.AddValidatorEvent{}, cbfttypes.RemoveValidatorEvent{}, cbfttypes.UpdateValidatorEvent{}, cbfttypes.ElectNextEpochVerifierEvent{})
