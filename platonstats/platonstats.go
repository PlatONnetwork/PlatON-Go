...
type PlatonStatsService struct {
	...
	ethapi          ethapi.Backend // Full Ethereum service if monitoring a full node
	datadir         string
...
func New(kafkaUrl, kafkaBlockTopic string, api ethapi.Backend, node *node.Node, datadir string) error {
	...
	node.RegisterLifecycle(platonStatsService)
	return nil
}

func (s *PlatonStatsService) BlockChain() ethapi.Backend {
	return s.ethapi
}

func (s *PlatonStatsService) ChainDb() ethdb.Database {
	return s.ethapi.ChainDb()
}
...
func (s *PlatonStatsService) Start() error {
...
func (s *PlatonStatsService) blockMsgLoop() {
	...
	for {
		...
		case <-s.quit:
			...
			return
		default:
			nextBlock, _ := s.BlockChain().BlockByNumber(context.Background(), rpc.BlockNumber(nextBlockNumber))
			if nextBlock != nil {
...
func (s *PlatonStatsService) reportBlockMsg(block *types.Block) error {
	...
	if block.NumberU64() == 0 {
		...
		receipts, _ = s.BlockChain().GetReceipts(context.Background(), block.Hash())
		exeBlockData = statsdb.Instance().ReadExeBlockData(block.Number())
		statData = statsdb.Instance().ReadStatData(block.Number())
	}
...
func (s *PlatonStatsService) getCode(to common.Address, blockNumber uint64) ([]byte, error) {
	state, _, err := s.ethapi.StateAndHeaderByNumber(nil, rpc.BlockNumber(blockNumber))
