...
def test_IV_001_002_010(global_test_env, staking_consensus_obj):
...
def test_IV_006_007_008(client_consensus_obj, get_generate_account):
    ...
    msg = client_consensus_obj.staking.withdrew_staking(StakingAddress)
    log.info(msg)
    ...
    msg = client_consensus_obj.staking.create_staking(0, StakingAddress, StakingAddress)
    assert msg["Code"] == 0
    address, _ = get_generate_account
    msg = client_consensus_obj.delegate.delegate(0, address)
    log.info(msg)
    assert msg["Code"] == 0
...
def test_IV_009(client_consensus_obj, get_generate_account):
    ...
    StakingAddress = client_consensus_obj.economic.cfg.DEVELOPER_FOUNDATAION_ADDRESS
    msg = client_consensus_obj.staking.edit_candidate(StakingAddress, address1)
    log.info(msg)
    assert msg["Code"] == 0
...
def test_P_014_015_019_024(client_noc_list_obj, get_generate_account):
    ...
    msg = client_noc_list_obj[0].staking.create_staking(0, address, address)
    log.info(msg)
    assert msg["Code"] == 0
    msg = client_noc_list_obj[0].staking.create_staking(0, address, address)
    log.info(msg)
    assert msg["Code"] == 301101
...
def test_P_016(client_new_node_obj, get_generate_account):
    ...
    address, _ = get_generate_account
    msg = client_new_node_obj.staking.create_staking(0, address, address, node_id=illegal_nodeID)
    log.info(msg)
    assert msg["Code"] == 301003
...
def test_P_017(client_new_node_obj, get_generate_account):
    ...
    address, _ = get_generate_account
    msg = client_new_node_obj.staking.create_staking(0, INCENTPEPOOL_ADDRESS, address)
    assert msg["Code"] == 0
...
def test_P_018(client_new_node_obj, get_generate_account):
    ...
    address, _ = get_generate_account
    msg = client_new_node_obj.staking.create_staking(0, FOUNDATION_ADDRESS, address)
    assert msg["Code"] == 0
...
def test_P_020_21(client_new_node_obj, get_generate_account):
    ...
    msg = client_new_node_obj.staking.create_staking(0, address, address, amount=amount - 1)
    log.info(msg)
    assert msg["Code"] == 301100
    cfg = {"gas": 1}
    status = 0
    try:
        msg = client_new_node_obj.staking.create_staking(0, address, address, transaction_cfg=cfg)
        log.info(msg)
    except:
        status = 1
    assert status == 1
...
def test_P_025(client_new_node_obj, get_generate_account, client_consensus_obj):
    ...
    msg = client_new_node_obj.staking.create_staking(0, address, address, program_version_sign=program_version_sign)
    log.info(msg)
    assert msg["Code"] == 301003
...
def test_P_029(client_new_node_obj, get_generate_account):
    ...
    assert msg["Code"] == 0
    msg = client_new_node_obj.staking.withdrew_staking(address)
    log.info(msg)
    assert msg["Code"] == 0
    msg = client_new_node_obj.staking.create_staking(0, address, address)
    log.info(msg)
...
def test_P_030(client_new_node_obj, get_generate_account):
    ...
    msg = client_new_node_obj.staking.create_staking(0, address, address)
    log.info(msg)
    assert msg["Code"] == 301101,"预期退质押未完成,再质押失败"
