...
@pytest.mark.P0
def test_EI_BC_001_009(client_new_node):
    """
...
def test_EI_BC_002(clients_new_node):
    ...
    # initiate a commission
    result = first_client.delegate.delegate(0, address, amount=von_amount(first_economic.delegate_limit, 5))
    assert_code(result, 0)
    time.sleep(5)
    result = second_client.delegate.delegate(0, address, amount=von_amount(second_economic.delegate_limit, 5))
    assert_code(result, 0)
    ...
    blocknum = result['Ret']['StakingBlockNum']
    result = second_client.delegate.withdrew_delegate(blocknum, address)
    assert_code(result, 0)
    first_delegate_epoch, first_cumulative_income = get_dividend_information(first_client, address)
    second_delegate_epoch, second_cumulative_income = get_dividend_information(second_client, address)
    assert first_delegate_epoch == 1, "ErrMsg: Last time delegate epoch {}".format(first_delegate_epoch)
    ...
    assert result['Ret'][0]['reward'] == 0, "ErrMsg: Withdraw commission award {}".format(result['Ret'][0]['reward'])
    result = second_client.delegate.withdraw_delegate_reward(address)
    assert_code(result, 0)
    first_delegate_epoch, first_cumulative_income = get_dividend_information(first_client, address)
    second_delegate_epoch, second_cumulative_income = get_dividend_information(second_client, address)
    assert first_delegate_epoch == 1, "ErrMsg: Last time delegate epoch {}".format(first_delegate_epoch)
...
@pytest.mark.P1
def test_EI_BC_004(clients_new_node):
    """
    ...
    result = second_client.delegate.delegate(0, address)
    assert_code(result, 0)
    ...
    assert second_cumulative_income == 0, "ErrMsg: Last time second cumulative income {}".format(second_cumulative_income)
    ...
    assert first_delegate_total_hes == first_economic.delegate_limit, "The total number of inactive nodes commissioned: {}".format(first_delegate_total_hes)
    ...
    assert second_delegate_total_hes == second_economic.delegate_limit, "The total number of inactive nodes commissioned: {}".format(second_delegate_total_hes)
    ...
    assert first_cumulative_income == first_commission_award, "ErrMsg: Last time first cumulative income {}".format(first_cumulative_income)
    ...
    assert first_delegate_total == von_amount(first_economic.delegate_limit, 2), "The total number of effective commissioned nodes: {}".format(
        first_delegate_total)
    assert first_delegate_total_hes == first_economic.delegate_limit, "The total number of inactive nodes commissioned: {}".format(
        first_delegate_total_hes)
    ...
    assert second_delegate_total == von_amount(second_economic.delegate_limit, 2), "The total number of effective commissioned nodes: {}".format(
        second_delegate_total)
    assert second_delegate_total_hes == second_economic.delegate_limit, "The total number of inactive nodes commissioned: {}".format(
        second_delegate_total_hes)
...
@pytest.mark.p1
def test_EI_BC_005(client_new_node):
    """
    锁仓金额首次委托，验证待领取的委托收益（单节点）
    :param client_new_node:
    :return:
    """
    client = client_new_node
    ...
    # initiate a commission
    result = client.delegate.delegate(1, address, amount=von_amount(economic.delegate_limit, 5))
    assert_code(result, 0)
    delegate_epoch, cumulative_income = get_dividend_information(client, address)
    assert delegate_epoch == 1, "ErrMsg: Last time delegate epoch {}".format(delegate_epoch)
    assert cumulative_income == 0, "ErrMsg: Last time cumulative income {}".format(cumulative_income)
...
@pytest.mark.P1
def test_EI_BC_006(clients_new_node):
    """
    ...
    # initiate a commission
    result = first_client.delegate.delegate(1, address)
    assert_code(result, 0)
    result = second_client.delegate.delegate(1, address)
    assert_code(result, 0)
    first_delegate_epoch, first_cumulative_income = get_dividend_information(first_client, address)
    second_delegate_epoch, second_cumulative_income = get_dividend_information(second_client, address)
    assert first_delegate_epoch == 1, "ErrMsg: Last time delegate epoch {}".format(first_delegate_epoch)
...
@pytest.mark.P0
def test_EI_BC_007(client_new_node):
    """
    ...
    economic = client.economic
    node = client.node
    address = create_restricting_plan(client_new_node)
    log.info("Create delegate account：{}".format(address))
    create_free_staking_node(client)
    log.info("Create pledge node id :{}".format(node.node_id))
    result = client.delegate.delegate(1, address)
    assert_code(result, 0)
    log.info("Commissioned successfully, commissioned amount：{}".format(economic.delegate_limit))
    delegate_epoch, cumulative_income = get_dividend_information(client, address)
    assert delegate_epoch == 1, "ErrMsg: Last time delegate epoch {}".format(delegate_epoch)
    ...
    last_delegate_epoch, delegate_total, delegate_total_hes, delegate_reward_total = get_delegate_relevant_amount_and_epoch(
        client)
    assert last_delegate_epoch == 2, "ErrMsg: Last time delegate epoch {}".format(last_delegate_epoch)
    assert delegate_total == economic.delegate_limit, "The total number of effective commissioned nodes: {}".format(
        delegate_total)
    assert delegate_total_hes == economic.delegate_limit, "The total number of inactive nodes commissioned: {}".format(
        delegate_total_hes)
    assert delegate_reward_total == 0, "Total delegated rewards currently issued by the candidate: {}".format(
    ...
    last_delegate_epoch, delegate_total, delegate_total_hes, delegate_reward_total = get_delegate_relevant_amount_and_epoch(
        client)
    assert last_delegate_epoch == 3, "ErrMsg: Last time delegate epoch {}".format(last_delegate_epoch)
    assert delegate_total == von_amount(economic.delegate_limit,
...
@pytest.mark.P1
def test_EI_BC_008(clients_new_node):
    """
...
@pytest.mark.P0
def test_EI_BC_010(client_new_node):
    """
...
@pytest.mark.P0
def test_EI_BC_11(client_new_node):
    """
...
@pytest.mark.P0
def test_EI_BC_012(client_new_node):
    """
...
@pytest.mark.P0
def test_EI_BC_028(client_new_node):
    """
    不同结算期赎回部分自由委托（生效期N赎回）
    :param client_new_node:
    :return:
    """
    client = client_new_node
...
@pytest.mark.P0
def test_EI_BC_030(client_new_node):
    """
    不同结算期赎回全部自由委托（生效期N赎回）
    :param client_new_node:
    :return:
    """
    client = client_new_node
    ...
    assert delegate_reward_total == 0, "Total delegated rewards currently issued by the candidate: {}".format(delegate_reward_total)
    result = client.ppos.getDelegateReward(address)
    assert result['Ret'][0]['reward'] == 0, "ErrMsg: Dividends currently available {}".format(result['Ret'][0]['reward'])


@pytest.mark.P0
def test_EI_BC_031(client_new_node):
    """
    跨结算期赎回全部自由委托（生效期N赎回）
    :param client_new_node:
    :return:
...
@pytest.mark.P0
def test_EI_BC_070(clients_new_node):
    """
    跨结算期多节点被委托，领取单个节点分红
    :param clients_new_node:
    :return:
...
def test_EI_BC_075(client_new_node):
    ...
    economic.wait_settlement_blocknum(node)
    log.info("Current settlement block height：{}".format(node.eth.blockNumber))
    second_commission_award = economic.calculate_delegate_reward(node, block_reward, staking_reward)
    result = client.delegate.withdraw_delegate_reward(address)
    assert_code(result, 0)
    result = client.ppos.getDelegateInfo(blocknum, address)
    assert_code(result, 301205)
    last_delegate_epoch, delegate_total, delegate_total_hes, delegate_reward_total = get_delegate_relevant_amount_and_epoch(
        client)
    assert last_delegate_epoch == 3, "ErrMsg: Last time delegate epoch {}".format(last_delegate_epoch)
    ...
    assert delegate_total_hes == 0, "The total number of inactive nodes commissioned: {}".format(
        delegate_total_hes)
    assert delegate_reward_total == commission_award + second_commission_award, "Total delegated rewards currently issued by the candidate: {}".format(
        delegate_reward_total)
    result = client.ppos.getDelegateReward(address)
    assert result['Ret'][0]['reward'] == 0, "ErrMsg: Dividends currently available {}".format(
        result['Ret'][0]['reward'])
