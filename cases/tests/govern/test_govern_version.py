...
import pytest
from tests.lib.utils import get_staking_address, get_client_obj, get_pledge_list, upload_platon, wait_block_number
...
def new_node_has_proposal(global_test_env, client_new_node_obj, client_verifier_obj, client_noconsensus_obj):
    ...
    if pip_obj.is_exist_effective_proposal():
        ...
        if proposalinfo.get('EndVotingBlock') - pip_obj.node.block_number < 2 * pip_obj.economic.consensus_size:
            ...
            result = pip_obj.submitVersion(pip_obj.node.node_id, str(time.time()), pip_obj.cfg.version5, 5,
                                           get_staking_address(pip_obj.node),
    ...
    result = pip_obj.submitVersion(pip_obj.node.node_id, str(time.time()), pip_obj.cfg.version5, 5,
                          get_staking_address(pip_obj.node), transaction_cfg=pip_obj.cfg.transaction_cfg)
    assert result.get('Code') == 0
    return client_new_node_obj.pip
...
def candidate_has_proposal(global_test_env, client_candidate_obj, client_verifier_obj, client_list_obj):
    ...
    if pip_obj.is_exist_effective_proposal():
        ...
        if proposalinfo.get('EndVotingBlock') - pip_obj.node.block_number < 2 * pip_obj.economic.consensus_size:
            ...
            candidate_node_pip_obj = get_client_obj(node_id_list[0])
        else:
            return client_candidate_obj
    result = pip_obj.submitVersion(pip_obj.node.node_id, str(time.time()), pip_obj.cfg.version5, 5,
                          pip_obj.node.staking_address, transaction_cfg=pip_obj.cfg.transaction_cfg)
    log.info('发起升级提案结果为{}'.format(result))
    assert result.get('Code') == 0
    return candidate_node_pip_obj
...
def test_VP_VE_001_to_VP_VE_004(no_version_proposal):
    ...
    assert result.get("ErrMsg") == "newVersion should larger than current active version"
...
def test_VP_WA_001(no_version_proposal):
    ...
    assert result.get('ErrMsg') == "Tx caller differ from staking"

def test_TP_WA_001(pip_obj):
    address, _ = pip_obj.economic.account.generate_account(pip_obj.node.web3, 10**18 * 10000000)
...
def test_VP_SU_001_VP_UN_001(submit_version):
    ...
    assert result.get('ErrMsg') == "another version proposal at voting stage"

def test_VP_UN_002(submit_version, pip_obj_list):
    pip_obj = submit_version
    proposalinfo = pip_obj.get_effect_proposal_info_of_vote()
    log.info('获取升级提案信息为{}'.format(proposalinfo))
    ...
    for nodeid in verifier_list:
        pip_obj_tmp = get_pip_obj(nodeid, pip_obj_list)
        ...
        result = pip_obj_tmp.vote(pip_obj_tmp.node_id, proposalinfo.get("ProposalID"), pip_obj_tmp.cfg.vote_option_yeas,
                                  pip_obj_tmp.node.staking_address, transaction_cfg=pip_obj_tmp.cfg.transaction_cfg)
        log.info('节点{}升级提案投票结果为{}'.format(pip_obj_tmp.node_id, result))
        assert result.get('Code') == 0
    wait_block_number(pip_obj, proposalinfo.get('EndVotingBlock'))
    ...
    assert result.get('ErrMsg') == "another version proposal at pre-active stage"

class TestEndVotingRounds():
    def test_VP_CR_001_VP_CR_002_VP_CR_007_TP_TE_002(self, pip_env, pip_obj):
        '''
        ...
        assert result.get('ErrMsg') == "endVotingRounds too large"
        ...
        assert result.get('ErrMsg') == "endVotingRounds too small"
    ...
    def test_VP_CR_003_VP_CR_004_VP_CR_007_TP_TE_003(self, pip_env, pip_obj):
        genesis = from_dict(data_class=Genesis, data=pip_env.genesis_config)
    ...
    def test_VP_CR_005_VP_CR_006_TP_TE_001(self, pip_env, pip_obj):
        genesis = from_dict(data_class=Genesis, data=pip_env.genesis_config)
        genesis.EconomicModel.Gov.VersionProposalVote_DurationSeconds = 3 * pip_obj.cfg.consensus_block
        genesis.EconomicModel.Gov.TextProposalVote_DurationSeconds = 5 * pip_obj.cfg.consensus_block
        pip_env.set_genesis(genesis.to_dict())
        pip_env.deploy_all()
...
class TestNoVerifierSubmitProposal():
    def test_VP_PR_002_TP_PR_002(self, new_node_pip_obj):
        address, _ = new_node_pip_obj.economic.account.generate_account(new_node_pip_obj.node.web3, 10**18 * 10000000)
        result = new_node_pip_obj.submitVersion(new_node_pip_obj.node.node_id, str(time.time()), new_node_pip_obj.cfg.version5, 1, address,
                                       transaction_cfg=new_node_pip_obj.cfg.transaction_cfg)
        log.info('新节点发起版本声明，结果为{}'.format(result))
        assert result.get('Code') == 302022
        assert result.get('ErrMsg') == "Tx caller is not verifier"

        result = new_node_pip_obj.submitText(new_node_pip_obj.node.node_id, str(time.time()), address,
                                       transaction_cfg=new_node_pip_obj.cfg.transaction_cfg)
        log.info('新节点发起版本声明，结果为{}'.format(result))
        assert result.get('Code') == 302022
        assert result.get('ErrMsg') == "Tx caller is not verifier"

    def test_VP_PR_001_TP_PR_001(self, candidate_node_pip_obj):
        pip_obj = candidate_node_pip_obj
        result = pip_obj.submitVersion(pip_obj.node.node_id, str(time.time()),
                                                pip_obj.cfg.version5, 1, pip_obj.node.staking_address,
                                                transaction_cfg=pip_obj.cfg.transaction_cfg)
        log.info('候选节点节点{}发起版本声明，结果为{}'.format(pip_obj.node.node_id, result))
        assert result.get('Code') == 302022
        assert result.get('ErrMsg') == "Tx caller is not verifier"
    ...
    def test_VP_PR_003_VP_PR_004_TP_PR_003_TP_PR_004(self, client_pip_obj):
        client_obj = client_pip_obj
        result = client_obj.staking.withdrew_staking(client_obj.node.staking_address)
        log.info('节点{}发起退质押结果为{}'.format(client_obj.node.node_id, result))
        assert result.get("Code") == 0
        log.info(client_obj.economic.account.find_pri_key(client_obj.node.staking_address))
        result = client_obj.pip.submitVersion(client_obj.node.node_id, str(time.time()), client_obj.pip.cfg.version5,
                                                   1, client_obj.node.staking_address,
                                              transaction_cfg=client_obj.pip.cfg.transaction_cfg)
        log.info('节点退出中，发起升级提案结果为{}'.format(result))
        assert result.get('Code') == 302020
        assert result.get('ErrMsg') == "verifier status is invalid"

        result = client_obj.pip.submitText(client_obj.node.node_id, str(time.time()), client_obj.node.staking_address,
                                           transaction_cfg=client_obj.pip.cfg.transaction_cfg)
...
class TestSubmitCancel():
    ...
    @pytest.mark.P2
    def test_CP_PR_003(self, client_pip_obj, submit_version):
        client_obj = client_pip_obj
        proposalinfo = client_obj.node
        result = client_obj.staking.withdrew_staking(client_obj.node.staking_address)
        log.info('节点{}发起退质押结果为{}'.format(client_obj.node.node_id, result))
        assert result.get("Code") == 0
        log.info(client_obj.economic.account.find_pri_key(client_obj.node.staking_address))
        result = client_obj.pip.submitCancel(client_obj.node.node_id, str(time.time()), 1, proposalinfo.get('ProposalID'),
