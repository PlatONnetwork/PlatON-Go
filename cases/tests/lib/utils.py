...
def get_node_obj(nodeid, node_obj_list):
    '''
    根据节点id获取节点对象
    :param nodeid:
    :param node_obj_list:
    ...
    for node_obj in node_obj_list:
        if nodeid == node_obj.node_id:
            return node_obj


def get_node_obj_list(nodeid_list, node_obj_list):
    '''
    根据节点id列表获取节点对象列表
    :param node_id_list:
    :param node_list:
    ...
    for nodeid in nodeid_list:
        new_node_obj_list.append(get_node_obj_list(nodeid, node_obj_list))
    return new_node_obj_list
...
def get_staking_address(node_obj):
    """
    获取质押钱包地址
    :return:
    """
    result = node_obj.ppos.getCandidateInfo(node_obj.node_id)
    candidate_info = result.get('Data')
    address = candidate_info.get('StakingAddress')
    return node_obj.web3.toChecksumAddress(address)
