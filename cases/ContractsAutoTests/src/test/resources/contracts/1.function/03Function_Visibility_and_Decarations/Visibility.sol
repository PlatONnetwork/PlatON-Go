...
contract Visibility {
    ...
    function fpri(uint a) private returns(uint) { 
        return a + 1; 
    }

    //external:外部函数是合约接口的一部分，所以我们可以从其它合约或通过交易来发起调用
    function fe(uint a) external returns(uint){ 
        return a + 2; 
    }
    
    //public:公开函数是合约接口的一部分，可以通过内部，或者消息来进行调用
    function fpub(uint a) public returns(uint) { 
        return a + 3; 
    }
...
contract Inter is Visibility {
    function g() public returns(uint result){
        //VisibilityTest visibilitytest = new VisibilityTest();
        uint result = add(1, 2);  // acces to internal member (from derivated to parent contract)
    }
}