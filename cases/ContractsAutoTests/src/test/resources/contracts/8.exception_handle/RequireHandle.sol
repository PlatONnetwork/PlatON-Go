...
contract RequireHandle {
     ...
     **/
    function functionCallEcecption() public {
    ...
    function outFunctionCallException() public {
        feed.nonCode.gas(800)();

    }
    //如果合约没有payable修饰符的public的函数在接收主币时（包括构造函数和回退函数）
    function nonPayableReceiveEthException() public {
        address payable to_pay =address(uint160(0x14723A09ACff6D2A60DcdF7aA4AFf308FDDC160C));
        to_pay.transfer(10);
    }
    //如果合约通过一个public的getter函数（public getter funciton）接收主币
    function publicGetterReceiveEthException() public {
        get_pay.transfer(101 ether);
    }
    //如果.transfer()执行失败
    function transferCallException() public payable {
        address payable to_pay =address(uint160(0x14723A09ACff6D2A60DcdF7aA4AFf308FDDC160C));
        to_pay.transfer(10);
    }
