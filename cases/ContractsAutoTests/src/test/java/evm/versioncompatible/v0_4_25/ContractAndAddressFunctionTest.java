...
public class ContractAndAddressFunctionTest extends ContractPrepareTest {
    ...
    public void testMicroLat() {
        try {
            ...
            Transfer transfer = new Transfer(web3j, transactionManager);
            TransactionReceipt receipt = transfer.sendFunds(contractAddress, BigDecimal.valueOf(1.00), Convert.Unit.MICROLAT, GAS_PRICE, GAS_LIMIT).send();
            if (StringUtils.equals(receipt.getStatus(), "0x1")) {
                PlatonGetBalance balance = web3j.platonGetBalance(contractAddress, DefaultBlockParameterName.LATEST).send();
    ...
    public void testMilliLat() {
        try {
            ...
            Transfer transfer = new Transfer(web3j, transactionManager);
            TransactionReceipt receipt = transfer.sendFunds(contractAddress, BigDecimal.valueOf(1.00), Convert.Unit.MILLILAT, GAS_PRICE, GAS_LIMIT).send();
            if (StringUtils.equals(receipt.getStatus(), "0x1")) {
                PlatonGetBalance balance = web3j.platonGetBalance(contractAddress, DefaultBlockParameterName.LATEST).send();
