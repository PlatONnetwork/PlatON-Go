    ...
    public void testReferenceDataTypeTuple() {
        ...
        try {
           ...
           //2、验证：tuple根据索引取值
            ...
            Byte actualValue2 = referenceDataTypeTupleContract.getTupleValueIndex2().send();
            ...
            collector.assertEqual(actualValue2,Byte.parseByte(expectValue2), "checkout execute success.");

            //3、验证：tuple元组初始化赋值方式二(使用make_tuple函数)
            TransactionReceipt  transactionReceipt1 = referenceDataTypeTupleContract.setInitTupleModeTwo(a,Byte.parseByte(b)).send();
            ...
            //4、验证:定义包含引用类型
            TransactionReceipt  transactionReceipt2 = referenceDataTypeTupleContract.setInitTupleModeThree(name,Long.parseLong(age)).send();
