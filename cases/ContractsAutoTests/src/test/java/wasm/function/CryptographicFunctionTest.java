...
//            String bech32Address = Bech32.addressEncode(NetworkParameters.TestNetParams.getHrp(), wasmAddress1.toString());
            byte[] hex = Bech32.addressDecode(wasmAddress1.toString());
            String resulraddr = "0x" + DataChangeUtil.bytesToHex(hex).toString();
            collector.assertEqual(publicKey, resulraddr);

            //platon_ripemd160
            byte[] wasmAddress2 = cryptographicFunction.call_platon_ripemd160(DataChangeUtil.stringToBytes32("hellow")).send();
            collector.logStepPass("call_platon_ripemd160函数返回值:" + wasmAddress2);
