...
func (cbft *Cbft) recoveryMsg(msg interface{}) error {
	...
	case *protocols.SendViewChange:
		...
		if should {
			node, err := cbft.validatorPool.GetValidatorByNodeID(m.ViewChange.Epoch, cbft.config.Option.Id)
			if err != nil {
				return err
			}
	...
	case *protocols.SendPrepareVote:
		...
		if should {
			...
			node, _ := cbft.validatorPool.GetValidatorByNodeID(m.Vote.Epoch, cbft.config.Option.Id)
