...
// findHighest finds the highest block from current start;
// If there are multiple highest blockExts, returns the one that singed by self;
// if none of blocks signed by self, returns the one that has most signs
...
func (cbft *Cbft) backTrackBlocks(start *BlockExt, end *BlockExt, includeEnd bool) []*BlockExt {
	cbft.log.Trace("back track blocks", "startHash", start.block.Hash(), "startNumber", start.block.NumberU64(), "startParentHash", start.block.ParentHash(), "endHash", end.block.Hash())
...
func (cbft *Cbft) blockReceiver(tmp *BlockExt) error {
	...
	path := block.NumberU64() - cbft.getRootIrreversible().number
	if path == 1 && !cbft.getRootIrreversible().isParent(block) {
	...
	if blockExt.inTree {
		...
		*/
		...
		if isLogical {
			...
			if newHighestLogical != nil {
				if blockExt.number == newHighestLogical.number && blockExt.block.Hash() == newHighestLogical.block.Hash() || blockExt.isAncestor(newHighestLogical) {
					//rearrange new logical path, and sign block if necessary
...
func (cbft *Cbft) ShouldSeal(curTime int64) (bool, error) {
	...
	if inturn {
		...
		if peersCount < cbft.getThreshold()-1 {
			cbft.log.Trace("connected peers not enough", "connectedPeersCount", peersCount)
			inturn = false
		}
	}
...
func (cbft *Cbft) CalcBlockDeadline() time.Time {
	...
	if nodeIdx >= 0 {
		if len(cbft.dpos.primaryNodeList) == 1 {
			return time.Now().Add(time.Duration(cbft.config.Period) * time.Second)
		}
...
func (cbft *Cbft) CalcNextBlockTime() time.Time {
	...
	return time.Now().Add(time.Duration(cbft.config.Period) * time.Second) // 1s
}
...
func (cbft *Cbft) calTurn(timePoint int64, nodeID discover.NodeID) bool {
	...
	if nodeIdx >= 0 {
		...
		if value > min && value < max {
			cbft.log.Trace("calTurn return true", "idx", nodeIdx, "min", min, "value", value, "max", max, "timePoint", common.MillisToString(timePoint), "startEpoch", common.MillisToString(startEpoch))
			return true
		} else {
			cbft.log.Trace("calTurn return false", "idx", nodeIdx, "min", min, "value", value, "max", max, "timePoint", common.MillisToString(timePoint), "startEpoch", common.MillisToString(startEpoch))
		}
	}
	return false
}
