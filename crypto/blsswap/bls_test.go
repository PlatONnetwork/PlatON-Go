...
func TestBlst(t *testing.T) {
	...
	//var sec SecretKey
	//sec.SetHexString("1ba530af467ed12ca19a85e237f3d87e8c2a2082ce27b0d3151a163007821991")
	//var pub PublicKey
	//pub.SetHexString("8065cbede15639eb0bf6449d37d1b2a2532f5beea3b4bef882f3798af39205229a44a574500382e07e29b630f23a1085")
	//var sig Sign
	//sig.SetHexString("92d9ac6f8de08004776e6d99f79b161f054ecaf30fa8f160e2028a993b23829a11dd733659cea875ad3d907a03a42ea7097d617d68755ba3a0451b524820f9056618273bb709951ecc53c4fd979db9dcc41c77c76f15a97a64bd1726bc9f3c19")
	//sig.Verify(&pub)
	Init(int(BLS12_381))
	m := "hello"
	var sec SecretKey
	sec.SetByCSPRNG()
	fmt.Println("sec:", sec.GetHexString())
	//fmt.Println("create public key")
	pub := sec.GetPublicKey()
	fmt.Println("pub:", hex.EncodeToString(pub.Serialize()))
	fmt.Println("pub:", pub.GetHexString())
	var mHash G1
	mHash.HashAndMapTo([]byte(m))
	fmt.Println("mHash:", hex.EncodeToString(mHash.Serialize()))
	fmt.Println("mHash:", mHash.GetString(16))
	sign := sec.Sign(m)
	fmt.Println("sign:", hex.EncodeToString(sign.Serialize()))
	fmt.Println("sign:", sign.GetHexString())
	if !sign.Verify(pub, m) {
		t.Error("Signature does not verify")
	}
}
