...
func (s *BlockChainAPI) GetMultiBalance(ctx context.Context, addresses []common.Address, blockNrOrHash rpc.BlockNumberOrHash) []*hexutil.Big {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil
	}
...
func (s *BlockChainAPI) GetProof(ctx context.Context, address common.Address, storageKeys []string, blockNrOrHash rpc.BlockNumberOrHash) (*AccountResult, error) {
	...
	for i, hexKey := range storageKeys {
		...
		if storageTrie != nil {
			...
			storageProof[i] = StorageResult{hexKey, (*hexutil.Big)(common.BytesToHash(state.GetState(address, key.Bytes())).Big()), toHexSlice(proof)}
		} else {
...
func (s *BlockChainAPI) GetStorageAt(ctx context.Context, address common.Address, hexKey string, blockNrOrHash rpc.BlockNumberOrHash) (hexutil.Bytes, error) {
	...
	res := state.GetState(address, key.Bytes())
	state.ClearParentReference()
...
func (s *BlockChainAPI) GetBlockReceipts(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]interface{}, error) {
	...
	for i, receipt := range receipts {
		signer := types.LatestSignerForChainID(txs[i].ChainId())
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.LAT)))
