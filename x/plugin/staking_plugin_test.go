...
func TestStakingPlugin_WithdrewDelegate(t *testing.T) {
	...
	*/
	...
	_, err = StakingInstance().WithdrewDelegation(state, blockHash2, blockNumber2,blockHash2, amount, addrArr[index+1],
	...
	issueIncome, err := StakingInstance().WithdrewDelegation(state, blockHash3, curBlockNumber, blockHash2, del.ReleasedHes, addrArr[index+1],
