...
func (rp *RestrictingPlugin) AddRestrictingRecord(sender common.Address, account common.Address, plans []byteutil.RestrictingPlan,
	...
	if len(bAccInfo) == 0 {
		...
		for i := 0; i < len(plans); i++ {
			...
			releaseEpochKey := xcom.GetReleaseEpochKey(epoch)
