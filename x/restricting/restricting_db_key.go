...
// GetRestrictingKey used for search restricting info. key: prefix + account
func GetRestrictingKey(account common.Address) []byte {
	return append(RestrictingKeyPrefix, account.Bytes()...)
}

// GetReleaseAmountKey used for search restricting entry info. key: prefix + epoch + account
...
// GetReleaseEpochKey used for search records at target epoch. key: prefix + epoch
func GetReleaseEpochKey(epoch uint64) []byte {
	return append(RestrictRecordKeyPrefix, common.Uint64ToBytes(epoch)...)
}

// GetReleaseAccountKey used for search the index of the restricting account in the released account
