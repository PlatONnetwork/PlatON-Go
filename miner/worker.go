...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64, commitBlock *types.Block, blockDeadline time.Time) {
	...
	if len(localTxs) > 0 {
		...
		if parallelMode {
			if failed, timeout := w.commitTransactionsWithHeaderParallelly(header, txs, interrupt, timestamp, blockDeadline); failed {
				return
			} else {
	...
	if !localTimeout && len(remoteTxs) > 0 {
		...
		if parallelMode {
			if failed, _ := w.commitTransactionsWithHeaderParallelly(header, txs, interrupt, timestamp, blockDeadline); failed {
				return
			}
		} else {
...
func (w *worker) commitTransactionsWithHeaderParallelly(header *types.Header, txs *types.TransactionsByPriceAndNonce, interrupt *int32, timestamp int64, blockDeadline time.Time) (failed bool, isTimeout bool) {
	// Short circuit if current is nil
	timeout := false
