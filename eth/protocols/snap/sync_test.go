...
func makeAccountTrieWithStorageWithUniqueStorage(accounts, slots int, code bool) (string, *trie.Trie, entrySlice, map[common.Hash]*trie.Trie, map[common.Hash]entrySlice) {
	...
	for i := uint64(1); i <= uint64(accounts); i++ {
		...
		_, stTrie, stEntries := makeStorageTrieWithSeed(common.BytesToHash(key), uint64(slots), i, db)
		stRoot := stTrie.Hash()
		stTrie.Commit(nil)
		value, _ := rlp.EncodeToBytes(&types.StateAccount{
			Nonce:            i,
		...
		storageTries[common.BytesToHash(key)] = stTrie.Copy()
...
func makeAccountTrieWithStorage(accounts, slots int, code, boundary bool) (string, *trie.Trie, entrySlice, map[common.Hash]*trie.Trie, map[common.Hash]entrySlice) {
	...
	for i := uint64(1); i <= uint64(accounts); i++ {
		...
		if boundary {
			_, stTrie, stEntries = makeBoundaryStorageTrie(common.BytesToHash(key), slots, db)
		} else {
			_, stTrie, stEntries = makeStorageTrieWithSeed(common.BytesToHash(key), uint64(slots), 0, db)
		}
		stRoot := stTrie.Hash()
		stTrie.Commit(nil)
...
func makeStorageTrieWithSeed(owner common.Hash, n, seed uint64, db *trie.Database) (string, *trie.Trie, entrySlice) {
...
// boundary hash.
func makeBoundaryStorageTrie(owner common.Hash, n int, db *trie.Database) (string, *trie.Trie, entrySlice) {
	var (
		entries    entrySlice
		boundaries []common.Hash
		trie, _    = trie.New(owner, common.Hash{}, db)
