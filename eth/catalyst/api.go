...
import (
	...
	chainParams "github.com/PlatONnetwork/PlatON-Go/params"
...
func (env *blockExecutionEnv) commitTransaction(tx *types.Transaction, coinbase common.Address) error {
	...
	receipt, err := core.ApplyTransaction(env.chain.Config(), env.chain, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, vmconfig)
	if err != nil {
...
func (api *consensusAPI) AssembleBlock(params assembleBlockParams) (*executableData, error) {
	...
	if config := api.eth.BlockChain().Config(); config.IsPauli(header.Number) {
