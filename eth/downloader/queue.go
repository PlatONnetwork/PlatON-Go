...
func newFetchResult(header *types.Header, fastSync bool) *fetchResult {
	...
	if fastSync && !header.EmptyReceipts() {
		//item.pending |= (1 << receiptType)	// The receipt is not synchronized in PlatON FastSync mode, so comment here
	}
	return item
}
...
func newQueue(blockCacheLimit int, decodeExtra decodeExtraFn) *queue {
