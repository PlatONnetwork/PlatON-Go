...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, bn *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network begin", "peer", p.id, "eth", p.version, "origin", origin, "pivot", pivot, "latest", latest.Number.Uint64(), "mode", d.mode)
...
func (d *Downloader) spawnSync(fetchers []func() error) error {
	...
	d.queue.Close()
	d.Cancel()
	log.Debug("Synchronising with the network end", "mode", d.mode, "headerNumber", d.blockchain.CurrentHeader().Number.Uint64(), "blockNumber", d.blockchain.CurrentBlock().NumberU64(), "fastNumber", d.blockchain.CurrentFastBlock().NumberU64())
	return err
}
...
func (d *Downloader) storagePposCachePoint(point uint64) bool {
	p := uint64(common.BaseSwitchWitness - common.BaseElection + 1)
