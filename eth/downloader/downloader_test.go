...
func newTester() *downloadTester {
	...
	tester.downloader = New(db, sdb, new(event.TypeMux), tester.chain, nil, tester.dropPeer, nil)
	tester.cbft = cbft
	return tester
}
...
func (dlp *downloadTesterPeer) RequestPPOSStorage(head uint64) error {
	dlp.dl.lock.RLock()
	defer dlp.dl.lock.RUnlock()
	currentHeader := dlp.chain.CurrentHeader()
	...
	if err := dlp.dl.downloader.DeliverPposStorage(dlp.id, [][2][]byte{}, true, 0, true, nil, 0); err != nil {
		dlp.Log().Error("[GetPPOSStorageMsg]send ppos meassage fail", "error", err, "kvnum", 0)
		return err
	}

	BlockStorage := make([]snapshotdb.BlockData, 0)

	for i := uint64(1); i <= head; i++ {
		b := dlp.chain.GetBlockByNumber(i)
		tmp := snapshotdb.NewBlockData(new(big.Int).SetUint64(i), b.ParentHash(), b.Hash())
		BlockStorage = append(BlockStorage, *tmp)
	}
	if err := dlp.dl.downloader.DeliverPposStorage(dlp.id, nil, false, 0, false, BlockStorage, 0); err != nil {
		dlp.Log().Error("[GetPPOSStorageMsg]send last ppos meassage fail", "error", err)
		return err
	}

	/*f := func(baseBlock uint64, iter iterator.Iterator, blocks []rlp.RawValue) error {
		dlp.Log().Debug("begin answerGetPPOSStorageMsgQueryV2", "blocks", len(blocks), "baseBlock", baseBlock)
		var (
			ps           [][2][]byte
			count        uint64
		...
		if err := dlp.dl.downloader.DeliverPposStorage(dlp.id, ps, true, count, true, nil, 0); err != nil {
			dlp.Log().Error("[GetPPOSStorageMsg]send ppos meassage fail", "error", err, "kvnum", count)
			return err
		}

		for _, block := range blocks {
			tmp := snapshotdb.BlockData{}
			if err := rlp.DecodeBytes(block, &tmp); err != nil {
				panic(err)
			}
			BlockStorage = append(BlockStorage, tmp)
		}
		if err := dlp.dl.downloader.DeliverPposStorage(dlp.id, nil, false, 0, false, BlockStorage, baseBlock); err != nil {
			dlp.Log().Error("[GetPPOSStorageMsg]send last ppos meassage fail", "error", err)
			return err
		}
		dlp.Log().Debug("end answerGetPPOSStorageMsgQueryV2", "blocks", len(blocks), "baseBlock", baseBlock)
		return nil
	}
	go func() {
		if err := dlp.dl.snapshotdb.WalkDB(head, f); err != nil {
			dlp.Log().Error("answer GetPPOSStorageMsgQueryV2  fail", "error", err)
		}
	}()*/
	return nil
}
...
func (dlp *downloadTesterPeer) RequestOriginAndPivotByCurrent(c uint64) error {
	...
	pivot, err := dlp.dl.snapshotdb.BaseNum()
	if err != nil {
		return errors.New("GetOriginAndPivot get snapshotdb baseNum fail")
	}

	phead := dlp.chain.GetHeaderByNumber(pivot.Uint64())

	oHead := dlp.chain.GetHeaderByNumber(c)

	go dlp.dl.downloader.DeliverOriginAndPivot(dlp.id, []*types.Header{oHead, phead})
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	...
	if rs := int(tester.chain.Engine().CurrentBlock().NumberU64()) + 1; rs != receipts {
...
func testEmptyShortCircuit(t *testing.T, protocol uint, mode SyncMode) {
	...
	for _, block := range chain.blocks[1:] {
		if mode != LightSync && (len(block.Transactions()) > 0 || len(block.ExtraData()) > 0) {
			bodiesNeeded++
		}
	}
	for _, block := range chain.blocks[1:] {
