...
func (tc *testChain) generate(n int, seed byte, parent *types.Block, heavy bool) {
	blocks, _ := core.GenerateChain(testGspec.Config, parent, consensus.NewFakerWithDataBase(testDB, parent), testDB, n, func(i int, block *core.BlockGen) {
		...
		if parent == tc.blocks[0] && i%22 == 0 {
			...
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), common.Address{seed}, big.NewInt(1000), params.TxGas*10, block.BaseFee(), nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
...
func newTestBlockchain(blocks []*types.Block) *core.BlockChain {
	...
	tbc.gen.Do(func() {
		...
		mockConsensus := consensus.NewFakerWithDataBase(db, testGenesis)
		chain, err := core.NewBlockChain(db, nil, testGspec, nil, mockConsensus, vm.Config{}, nil, nil)
		if err != nil {
			panic(err)
		}
		mockConsensus.SetChain(chain)
