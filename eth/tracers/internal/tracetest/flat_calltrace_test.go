...
func flatCallTracerTestRunner(tracerName string, filename string, dirPath string, t testing.TB) error {
	...
	signer := types.MakeSigner(test.Genesis.Config, new(big.Int).SetUint64(uint64(test.Context.Number)), true)
	...
	if !jsonEqualFlat(ret, test.Result) {
		...
		// 	t.FailNow()
		// }
		//for _, rett := range *ret {
		//	t.Logf("trace result address: %s", rett.Result.Address.Hex())
		//}
