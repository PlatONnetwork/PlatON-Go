...
func (b *testBackend) StateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, func(), error) {
	...
	signer := types.MakeSigner(b.chainConfig, false, false)
	for idx, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(context, txContext, nil, statedb, b.chainConfig, vm.Config{})
...
func TestTraceCall(t *testing.T) {
	...
	genBlocks := 10
	signer := types.MakeSigner(params.TestChainConfig, false, false)
