...
func TestMigrateStorage(t *testing.T) {
	...
	// }
	//
	state.MigrateStorage(from, to, false)
	...
	for _, key := range [][]byte{[]byte("a"), []byte("b"), []byte("c"), []byte("d"), []byte("e"), []byte("f")} {
		...
		switch string(key) {
		case "a":
			assert.Equal(t, "I am  A of to", string(value))
		case "b":
			assert.Equal(t, "I am  B of to", string(value))
		case "c":
			assert.Equal(t, "fromC2", string(value))
		case "d":
			assert.Equal(t, "I am  D of to", string(value))
		case "e":
			assert.Equal(t, "I am  E2 of to", string(value))
		case "f":
			assert.Equal(t, "I am  F of to", string(value))
		}
	}
}
