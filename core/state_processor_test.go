...
func TestStateProcessorErrors(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(db, nil, gspec.Config, consensus.NewFaker(), vm.Config{}, nil, nil)
	)
	...
	for i, tt := range []struct {
		...
		block := GenerateBadBlock(genesis, consensus.NewFaker(), tt.txs)
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions) *types.Block {
	header := &types.Header{
		...
		GasLimit:   CalcGasLimit(parent, parent.GasLimit()),
	...
	return types.NewBlock(header, txs, receipts, new(trie.Trie))
}
