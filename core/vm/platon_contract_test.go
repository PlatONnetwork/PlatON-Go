...
var (
	nodeIdArr = []discv5.NodeID{
		discv5.MustHexID("0x362003c50ed3a523cdede37a001803b8f0fed27cb402b3d6127a1a96661ec202318f68f4c76d9b0bfbabfd551a178d4335eaeaa9b7981a4df30dfc8c0bfe3384"),
		discv5.MustHexID("0xced880d4769331f47af07a8d1b79de1e40c95a37ea1890bb9d3f0da8349e1a7c0ea4cadbb9c5bf185b051061eef8e5eadca251c24e1db1d9faf0fb24cbd06f9a"),
		discv5.MustHexID("0xda56501a77fc1dfe0399b81f3909061d9a176cb9433fab4d3dfb1a10344c243274e38155e18878c7a0b3fcdd6182000c7784a95e2c4d9e0691ce67798624786e"),
		discv5.MustHexID("0x89a4409abe1ace8b77c4497c2073a8a2046dbdabb58c8bb58fe73926bbdc572fb848d739b1d2d09dd0796abcc1ed8d9a33bb3ef0a6c2e106e408090df179b041"),
		discv5.MustHexID("0x65e2ab09161e32e6d07d82adaa416ee6d41d617c52db20e3145a4d1b7d396af38d095c87508ad5bb35df741513bdc4bf12fec215e58450e255f05d194d41d089"),
		discv5.MustHexID("0x9bfacd628f3adb0f94e8b3968064d5248fa18efa75c680fdffea3af2575406461f3395817dd2a1be07a79bd81ffa00f57ad82286061d4a6caceece048e352380"),
		discv5.MustHexID("0x1e07d66b56bbc931ddce7cc5b9f55672d7fe4e19897a42f19d4ad7c969435cad652d720401d68f5769e245ec0f4e23362c8b1b062771d614876fdbb875ba9d44"),
		discv5.MustHexID("0x11a315747ce79cdf3d6aaf87ff2b6897950a20bda281838f922ea9407736fec9029d85f6202fd059a57a9119d05895402e7570948ae759cb093a54c3da9e0a4a"),
...
func build_staking_data(genesisHash common.Hash) {
	...
	id_A := enode.NodeIDToIDV4(nodeId_A)

	nodeId_B := nodeIdArr[1]
	id_B := enode.NodeIDToIDV4(nodeId_B)

	nodeId_C := nodeIdArr[2]
	id_C := enode.NodeIDToIDV4(nodeId_C)
	...
	stakingDB.SetCanPowerStore(blockHash, id_A, c1)
	stakingDB.SetCanPowerStore(blockHash, id_B, c2)
	stakingDB.SetCanPowerStore(blockHash, id_C, c3)

	stakingDB.SetCandidateStore(blockHash, id_A, c1)
	stakingDB.SetCandidateStore(blockHash, id_B, c2)
	stakingDB.SetCandidateStore(blockHash, id_C, c3)
...
func build_staking_data_new(chain *mock.Chain) {
	...
	err = stakingDB.SetCanPowerStore(chain.CurrentHeader().Hash(), id_A, c1)
	if err != nil {
		fmt.Println("err:", err)
	}
	stakingDB.SetCanPowerStore(chain.CurrentHeader().Hash(), id_B, c2)
	stakingDB.SetCanPowerStore(chain.CurrentHeader().Hash(), id_C, c3)

	stakingDB.SetCandidateStore(chain.CurrentHeader().Hash(), id_A, c1)
	stakingDB.SetCandidateStore(chain.CurrentHeader().Hash(), id_B, c2)
	stakingDB.SetCandidateStore(chain.CurrentHeader().Hash(), id_C, c3)
...
func build_staking_data_more(chain *mock.Chain) {
	...
	for i := 0; i < 1000; i++ {
		...
		var id enode.ID
		var v5 discv5.NodeID
		var addr common.Address

		if i < 25 {
			v5 = nodeIdArr[i]
			id = enode.NodeIDToIDV4(v5)
			ar, _ := xutil.NodeId2Addr(id)
			addr = common.Address(ar)
		} else {
			id, idv5, ar, err := randBuildFunc()
			if nil != err {
				return
			}
			id = id
		...
		err := stakingDB.SetCanPowerStore(chain.CurrentHeader().Hash(), canTmp.ID, canTmp)
		if err != nil {
			fmt.Println("stakingDB.SetCanPowerStore error:", err)
		}
		err = stakingDB.SetCandidateStore(chain.CurrentHeader().Hash(), canTmp.ID, canTmp)
		if err != nil {
