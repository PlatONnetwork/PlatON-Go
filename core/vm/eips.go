...
func opTload(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	val := interpreter.evm.StateDB.GetTransientState(scope.Contract.Address(), hash.Bytes())
	loc.SetBytes(val)
	return nil, nil
}
...
func opTstore(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	loc := scope.Stack.pop()
	val := scope.Stack.pop()
	interpreter.evm.StateDB.SetTransientState(scope.Contract.Address(), common.Hash(loc.Bytes32()).Bytes(), common.Hash(val.Bytes32()).Bytes())
	return nil, nil
}
