...
func fillPool(t testing.TB, pool *LegacyPool) {
	...
	for i := 0; i < 384; i++ {
		...
		for j := 0; j < int(pool.config.AccountSlots); j++ {
			executableTxs = append(executableTxs, pricedTransaction(uint64(j), 100000, big.NewInt(300), key, pool.chainconfig.PIP7ChainID))
		}
	}
...
func TestTransactionFutureAttack(t *testing.T) {
	...
	// Now, future transaction attack starts, let's add a bunch of expensive non-executables, and see if the pending-count drops
	{
		...
		for j := 0; j < int(pool.config.GlobalSlots+pool.config.GlobalQueue); j++ {
			futureTxs = append(futureTxs, pricedTransaction(1000+uint64(j), 100000, big.NewInt(500), key, pool.chainconfig.PIP7ChainID))
		}
		for i := 0; i < 5; i++ {
...
func TestTransactionZAttack(t *testing.T) {
	...
	for j := 0; j < int(pool.config.GlobalQueue); j++ {
		...
		key, _ := crypto.GenerateKey()
		pool.currentState.AddBalance(crypto.PubkeyToAddress(key.PublicKey), big.NewInt(100000000000))
		futureTxs = append(futureTxs, pricedTransaction(1000+uint64(j), 21000, big.NewInt(500), key, pool.chainconfig.PIP7ChainID))
		pool.AddRemotesSync(futureTxs)
	}
...
func BenchmarkFutureAttack(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		futureTxs = append(futureTxs, pricedTransaction(1000+uint64(n), 100000, big.NewInt(500), key, pool.chainconfig.PIP7ChainID))
	}
	b.ResetTimer()
	for i := 0; i < 5; i++ {
		pool.AddRemotesSync(futureTxs)
