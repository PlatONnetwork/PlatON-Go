...
var ErrPlatONTxDataInvalid = errors.New("the tx data is invalid")
...
// LegacyPool contains all currently known legacy-type transactions.
...
// New creates a new legacy transaction pool to gather, sort and filter inbound
// transactions from the network.
func New(config txpool.Config, chainconfig *params.ChainConfig, chain BlockChain) *LegacyPool {
...
func (pool *LegacyPool) ResetBlock(newBlock *types.Block) {
	startTime := time.Now()
	if pool == nil {
...
func (pool *LegacyPool) ForkedReset(newHeader *types.Header, rollback []*types.Block) {
	if len(rollback) == 0 {
		return
	}
	...
	pool.currentHead.Store(newHeader)
...
func (pool *LegacyPool) Close() error {
	// Unsubscribe all subscriptions registered from legacypool
	pool.scope.Close()
...
func (pool *LegacyPool) SubscribeNewTxsEvent(ch chan<- core.NewTxsEvent) event.Subscription {
...
func (pool *LegacyPool) PendingPlain(enforceTips, limited bool) map[common.Address]types.Transactions {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *LegacyPool) validateTxBasics(tx *types.Transaction, local bool) error {
	...
	opts := &txpool.ValidationOptions{
		...
		EIP2718: pool.eip2718.Load(),
		EIP1559: pool.eip1559.Load(),
		Hawking: pool.hawking.Load(),
	}
	if local {
		opts.MinTip = new(big.Int)
	}
	if err := txpool.ValidateTransaction(tx, nil, nil, nil, head, pool.signer, opts); err != nil {
		return err
	}
	return nil
}
...
func (pool *LegacyPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.To() != nil {
		if err := core.GetReactorInstance().VerifyTx(tx, *tx.To(), pool.chainconfig.Rules(pool.resetHead.Number)); err != nil {
...
//
// Note, this method assumes the pool lock is held!
func (pool *LegacyPool) promoteTx(addr common.Address, list *list, hash common.Hash, tx *types.Transaction) bool {
...
func (pool *LegacyPool) Statuses(hashes []common.Hash) []txpool.TxStatus {
	...
	for i, hash := range hashes {
		...
		// checking pool.Get and obtaining the lock. In that case, txpool.TxStatusUnknown is correct
		pool.mu.RUnlock()
	}
	return status
}
...
func (pool *LegacyPool) Count() int {
...
func (pool *LegacyPool) resetSigner(blockNumber *big.Int, statedb *state.StateDB) {
